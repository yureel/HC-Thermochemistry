...
class AtomFeatureVector:
    ...
    def construct_vector(self):
        ...
        self.vector += [1 if self.atom.GetFormalCharge() > 0 else 0]
        ...
        self.vector += [self.distance_to_charge()]
    ...
    def distance_to_charge(self):
        id_ref = self.atom.GetIdx()
        id = self.atom.GetIdx()
        for atom_i in self.mol.GetAtoms():
            if atom_i.GetFormalCharge() > 0.5:
                id = atom_i.GetIdx()
                break
...
class BondFeatureVector:
    ...
    def construct_vector(self):
        ...
        self.vector += [self.bond.GetBeginAtom().GetFormalCharge()+self.bond.GetEndAtom().GetFormalCharge()]
