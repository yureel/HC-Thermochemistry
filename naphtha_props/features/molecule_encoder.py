from .feature_vectors_carb import *
...
class MolEncoder:
    ...
    def __init__(self, mol: Chem.rdchem.Mol, property: str = "solvation", dummy=False):
        ...
        for atom in self.mol.GetAtoms():
            ...
            af = AtomFeatureVector(atom, self.mol, property=self.property)
            f_atoms.append(af.construct_vector())
            self.fa_size = af.get_atom_feature_dim()

        #self.f_atoms = [self.f_atoms[i] for i in range(self.n_atoms)] #check what they are doing here,does not seem to change anything
        ...
        #self.a2b = [a2b[a] + [0] * (self.max_num_bonds - len(a2b[a])) for a in range(self.n_atoms)]
        self.a2b = a2b
        self.b2a = b2a
        self.b2revb = b2revb
