...
class MPN(nn.Module):
    ...
    def __init__(self, depth=3, hidden_size=100, dropout=0.1,
                 ...
                 aggregation: str = 'mean', aggregation_norm=None):
        ...
        self.aggregation_norm = aggregation_norm if aggregation is 'norm' else None
    ...
    def forward(self, data: DataTensor):
        ...
        #  comment out to see if this uses a lot of memory! (only needed for attention, but we dont really use this anymore)
        # data.set_current_a_hiddens(atom_hiddens)
        ...
        for i, (a_start, a_size) in enumerate(ascope):
            if a_size == 0:
                ...
                # mol_vec = mol_vec.max(dim=0).values
                # mol_vec = mol_vec.mean(dim=0)
                mol_vecs.append(mol_vec)
                # if count<num_mols-1:
                #    count+=1
                #    umv.append(mol_vec)
                #    uav.append(cur_hiddens)

                # else:
                # datapoints[int(np.floor(i / num_mols))].set_updated_mol_vecs(umv)
                # datapoints[int(np.floor(i / num_mols))].set_updated_atom_vecs(uav)
                #    umv = []
                #    uav = []
                #    count=0
